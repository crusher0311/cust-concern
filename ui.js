...
export function copyConversation() {
    ...
    getPredictions(prompt).then(cleanedConversation => {
        // Single-confirm copy: show confirm once then copy all parts sequentially
        ...
        if (conversationParts.length === 0) {
            alert('No conversation text available to copy.');
            return;
        }

        if (!confirm(`This will copy the cleaned conversation to the clipboard in ${conversationParts.length} part(s). Proceed?`)) {
            return;
        }

        // Copy parts sequentially without additional prompts
        let idx = 0;
        function copySequential() {
            if (idx >= conversationParts.length) {
                alert('All conversation parts have been copied to the clipboard.');
                return;
            }
            navigator.clipboard.writeText(conversationParts[idx]).then(() => {
                idx++;
                // Small delay to ensure clipboard is updated between writes
                setTimeout(copySequential, 150);
            }).catch(err => {
...
export function sendToTekmetric() {
    ...
    // Show preview modal with cleaned conversation and let user edit/confirm
    getPredictions(prompt).then(cleanedConversation => {
        const previewModal = document.getElementById('previewModal');
        const previewText = document.getElementById('previewText');
        previewText.value = cleanedConversation;
        previewModal.style.display = 'flex';

        // Wire preview buttons
        document.getElementById('previewClose').onclick = () => { previewModal.style.display = 'none'; };
        document.getElementById('previewEditButton').onclick = () => {
            // Keep modal open for editing - focus the textarea
            previewText.focus();
        };
        document.getElementById('previewSendButton').onclick = () => {
            const finalText = previewText.value;
            previewModal.style.display = 'none';
            chrome.runtime.sendMessage({
                action: 'sendToTekmetric',
                cleanedConversation: finalText
            }, (response) => {
